...
- **THEN** it SHALL return a `ReconcileResult` with `requeue: bool` and `requeue_after: Option<Duration>`
...
- **WHEN** reconciler returns `ReconcileResult { requeue: true, requeue_after: Some(Duration::from_secs(30)) }`
- **THEN** the work queue SHALL requeue the key after 30 seconds

#### Scenario: Reconciler requests immediate requeue
- **WHEN** reconciler returns `ReconcileResult { requeue: true, requeue_after: None }`
- **THEN** the work queue SHALL requeue the key immediately (subject to rate limiting)
