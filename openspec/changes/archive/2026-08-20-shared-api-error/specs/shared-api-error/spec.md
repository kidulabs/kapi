...
### Requirement: ApiError has 15 named variants plus Unknown
...
- **THEN** it SHALL contain exactly 15 named variants plus the `Unknown` catch-all
...
### Requirement: CoreError is absorbed into ApiError
The existing `CoreError` enum in `kapi-core` (with variants `InvalidFieldSelector` and `InvalidLabelSelector`) SHALL be deprecated and its variants absorbed into `ApiError::InvalidRequest`.

#### Scenario: CoreError is deprecated
- **WHEN** code uses `CoreError`
- **THEN** the compiler SHALL emit a deprecation warning
- **THEN** the code SHALL be migrated to use `ApiError::InvalidRequest`
