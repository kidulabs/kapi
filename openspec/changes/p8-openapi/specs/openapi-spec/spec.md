...
- **WHEN** the spec is generated
- **THEN** `POST /apis/kapi.io/v1/Schema` has a request body schema referencing `SchemaData`

#### Scenario: Schema GET path has correct response
- **WHEN** the spec is generated
- **THEN** `GET /apis/kapi.io/v1/Schema/{name}` has a 200 response referencing `SchemaStoredObject`
...
- **THEN** the spec contains:
  ...
  - `POST /apis/example.io/v1/Widget` (create)
...
- **THEN** the POST request body references the kind's data component (e.g., `WidgetExampleIo`)
