...
- **WHEN** user runs `kapibuild controller generate` for a kind that doesn't exist in api/
...
The system SHALL generate controller code that uses TypedClient<Widget> for all CRUD operations instead of raw kapi-client.
