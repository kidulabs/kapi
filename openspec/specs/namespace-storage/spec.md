...
Define namespace-aware storage keys, store trait parameters, and pagination tokens for cross-namespace object storage.
...
The SQLite `objects` table SHALL include a `namespace` column of type `TEXT NOT NULL DEFAULT ''`. Cluster-scoped objects SHALL have `namespace = ''` (empty string). The primary key SHALL be `(resource_group, api_version, resource_kind, namespace, name)`.

#### Scenario: SQLite schema with namespace column
- **WHEN** the SQLite schema is initialized
- **THEN** the `objects` table SHALL have a `namespace TEXT NOT NULL DEFAULT ''` column
- **AND** the primary key SHALL include `namespace`

#### Scenario: SQLite stores cluster-scoped objects with empty namespace
- **WHEN** a cluster-scoped object is created
- **THEN** the `namespace` column SHALL be `''` (empty string)
