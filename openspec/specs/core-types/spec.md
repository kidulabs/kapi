...
The system SHALL produce `ApiError::InvalidRequest { what: "field selector", message }` for invalid field selector errors.

#### Scenario: InvalidFieldSelector returns 400
- **WHEN** an `ApiError::InvalidRequest { what: "field selector", message }` is returned from a handler
- **THEN** the HTTP response SHALL be 400 Bad Request with code `"InvalidRequest"` and details `{"what": "field selector", "message": "..."}`

**Note**: `FieldSelector::parse()` returns `ApiError::InvalidRequest` directly. The `CoreError` type has been removed.
