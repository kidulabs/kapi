...
The system SHALL define a `StoredObject` struct containing `key: ResourceKey`, `name: String`, `data: UserData`, `resource_version: u64`, `created_at: DateTime<Utc>`, and `updated_at: DateTime<Utc>`.

#### Scenario: Object carries versioning timestamps
- **WHEN** an object is created or updated
- **THEN** `created_at` and `updated_at` SHALL be populated by the storage layer

#### Scenario: Resource version for optimistic concurrency
- **WHEN** an object is created or updated
- **THEN** `resource_version` SHALL be updated by the storage layer
- **WHEN** an update request includes an `expected_resource_version`
- **THEN** the storage layer SHALL reject the update if `expected_resource_version != resource_version`, returning a `Conflict` error
