...
The registry SHALL cache status validators alongside spec validators. The cache key for status validators SHALL be `{kind}.{group}.status`.
...
When a Schema with `statusSchema` is created or updated, the status validator SHALL be compiled and cached. The cache key for status validators SHALL be `{kind}.{group}.status`. When a Schema is deleted, both spec and status cache entries SHALL be evicted.
...
- **THEN** the spec entry is removed from the cache and the status entry `"Widget.example.io.status"` is also removed
