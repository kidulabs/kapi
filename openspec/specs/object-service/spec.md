...
- `event_bus: EventBus` — the per-kind event bus for watch notifications
- `meta_validator: jsonschema::Validator` — compiled meta-schema for Schema validation
- `schema_cache: DashMap<String, Arc<jsonschema::Validator>>` — compiled user schemas keyed by schema name (e.g., `"Widget.example.io"`)
