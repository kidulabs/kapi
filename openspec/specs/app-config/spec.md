...
Define the `AppConfig` struct that encapsulates all server configuration (port, store, event bus) and serves as the single entry point for constructing a kapi server.
