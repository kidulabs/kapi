...
Define the status subresource pattern that allows controllers to update observed state without conflicting with spec updates. The status subresource is available only for kinds that have a `statusSchema` defined in their Schema registration.
