...
echo "With annotations:"; curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/ann-with-$TEST_RUN" | python3 -c "import sys,json;print(json.load(sys.stdin)['metadata'].get('annotations',{}))"
echo "Without annotations:"; curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/ann-without-$TEST_RUN" | python3 -c "import sys,json;print(json.load(sys.stdin)['metadata'].get('annotations',{}))"
echo "T38_PASS"

echo "========== TEST 39: Annotations update =========="
get_system_fields "ann-with-$TEST_RUN"

curl -s -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/ann-with-$TEST_RUN" \
  -H "Content-Type: application/json" \
...
echo "After update:"; curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/ann-with-$TEST_RUN" | python3 -c "import sys,json;print(json.load(sys.stdin)['metadata'].get('annotations',{}))"
echo "T39_PASS"
