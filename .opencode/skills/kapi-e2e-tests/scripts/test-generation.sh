...
curl -s -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/gen-meta-$TEST_RUN" \
  -H "Content-Type: application/json" \
...
CURRENT=$(curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/gen-meta-$TEST_RUN")
...
curl -s -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/gen-meta-$TEST_RUN/status" \
  -H "Content-Type: application/json" \
...
curl -s -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/gen-indep-$TEST_RUN" \
  -H "Content-Type: application/json" \
...
curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/gen-indep-$TEST_RUN" | python3 -c "import sys,json;obj=json.load(sys.stdin);print(f'  generation={obj[\"system\"][\"generation\"]}, resourceVersion={obj[\"system\"][\"resourceVersion\"]}')"
...
curl -s -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/gen-indep-$TEST_RUN/status" \
  -H "Content-Type: application/json" \
...
curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/gen-indep-$TEST_RUN" | python3 -c "
import sys,json;obj=json.load(sys.stdin)
gen=obj['system']['generation']; rv=obj['system']['resourceVersion']
print(f'  generation={gen}, resourceVersion={rv}')
