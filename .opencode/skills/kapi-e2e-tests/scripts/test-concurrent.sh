...
curl -s -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/concurrent-$TEST_RUN/status" \
  -H "Content-Type: application/json" \
  -d '{"status":{"phase":"Running"}}' > /dev/null

get_system_fields "concurrent-$TEST_RUN"
curl -s -w "\nHTTP_STATUS: %{http_code}\n" -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/concurrent-$TEST_RUN" \
  -H "Content-Type: application/json" \
...
curl -s -w "\nHTTP_STATUS: %{http_code}\n" -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/no-event-$TEST_RUN" \
  -H "Content-Type: application/json" \
