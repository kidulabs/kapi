...
echo "With labels:"; curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/lbl-with-$TEST_RUN" | python3 -c "import sys,json;print(json.load(sys.stdin)['metadata'].get('labels',{}))"
echo "Without labels:"; curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/lbl-without-$TEST_RUN" | python3 -c "import sys,json;print(json.load(sys.stdin)['metadata'].get('labels',{}))"
echo "T5_PASS: labels created correctly"

echo "========== TEST 6: Labels update =========="
get_system_fields "lbl-with-$TEST_RUN"

curl -s -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/lbl-with-$TEST_RUN" \
  -H "Content-Type: application/json" \
...
echo "After update:"; curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget/lbl-with-$TEST_RUN" | python3 -c "import sys,json;print(json.load(sys.stdin)['metadata'].get('labels',{}))"
...
echo "Listing labels:"; curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/namespaces/default/Widget" | python3 -c "
