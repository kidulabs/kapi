...
COUNT=$(curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/Widget?fieldSelector=metadata.name=list-field-foo-$TEST_RUN" | python3 -c "import sys,json; print(len(json.load(sys.stdin)['items']))")
...
curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/Widget?labelSelector=app=nginx" | python3 -c "
...
COUNT=$(curl -s "http://localhost:8080/apis/example.io.$TEST_RUN/v1/Widget?fieldSelector=metadata.name=list-both-target-$TEST_RUN&labelSelector=app=nginx" | python3 -c "import sys,json; print(len(json.load(sys.stdin)['items']))")
echo "Items: $COUNT (expected 1)"
...
CODE=$(curl -s -o /dev/null -w "%{http_code}" "http://localhost:8080/apis/example.io.$TEST_RUN/v1/Widget?fieldSelector=metadata.namespace=default")
