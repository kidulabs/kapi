...
curl -s -w "\nHTTP_STATUS: %{http_code}\n" -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/Widget/invalid-status-$TEST_RUN/status" \
  -H "Content-Type: application/json" \
  -d '{"status":{"phase":123}}'
echo "T27_PASS"
...
curl -s -w "\nHTTP_STATUS: %{http_code}\n" -X PUT "http://localhost:8080/apis/example.io.$TEST_RUN/v1/Widget/nonexistent-$TEST_RUN/status" \
  -H "Content-Type: application/json" \
  -d '{"status":{"phase":"Running"}}'
echo "T28_PASS"
