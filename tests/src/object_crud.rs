...
pub async fn test_list_single_page(app: &TestApp) -> Result<(), String> {
    ...
    assert!(list["continue_token"].is_null(), "expected no continue token");
...
pub async fn test_list_two_pages(app: &TestApp) -> Result<(), String> {
    ...
    assert!(page2["continue_token"].is_null(), "page2 should have no continue token");
...
pub async fn test_list_resume_position(app: &TestApp) -> Result<(), String> {
    ...
    let names1: Vec<&str> = page1["items"]
        .as_array()
        .map(|arr| arr.iter().filter_map(|item| item["metadata"]["name"].as_str()).collect())
        .unwrap_or_default();
...
pub async fn test_list_exhausted(app: &TestApp) -> Result<(), String> {
    ...
    assert!(list["continue_token"].is_null(), "last page should have no continue token");
