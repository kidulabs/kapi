...
| POST | `/Schema` | Register a new JSON Schema (validated against meta-schema). Name is auto-generated as `{targetKind}.{targetGroup}` — client does not supply `metadata.name` |
...
POST /apis/kapi.io/v1/Schema
  ...
  │  Name is generated by handler as {targetKind}.{targetGroup} from body fields
  │
  ▼ Handler: extract targetKind + targetGroup from body, generate name
  │
  ...
  │
  ▼ Response: 201 Created + StoredObject JSON with generated metadata.name
...
### P-Future — Schema Validation Feature

Schema validation (the feature itself, beyond meta-schema validation) is deferred to a future change. This refers to the ability for the system to validate registered schemas against additional constraints beyond the meta-schema, such as:

- **Semantic validation**: Ensuring schema references (`$ref`) resolve correctly, checking for circular references, validating schema complexity limits
- **Policy enforcement**: Restricting certain schema constructs (e.g., disallowing `additionalProperties: true`, limiting nesting depth)
- **Compatibility checks**: Detecting breaking changes when updating an existing schema

Currently, schema registration only validates against the meta-schema (structural shape: `targetGroup`, `targetVersion`, `targetKind`, `jsonSchema`) and attempts to compile the `jsonSchema` via the `jsonschema` crate. This catches malformed schemas but does not enforce semantic or policy constraints.
